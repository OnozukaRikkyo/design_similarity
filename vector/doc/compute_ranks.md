...
**レコード数（1,447）はインデックス件数（959）を超えるが、これは正常。**

- **インデックス件数（959）**: rank_index に収録されたユニーク特許数（グラフの**節点**数）
- **レコード数（1,447）**: 処理した引用ペア数（グラフの**辺**数）

同一の特許が複数の異なるペアに source または target として登場するため、辺の数が節点数を超える。

```
D18 perspective（2026-05-23 確認）:
  レコード数（辺）:             1,447
  ユニーク source 数:             647
  ユニーク target 数:             654
  ユニーク特許数 (source∪target): 959  ← rank_index と一致
  source 最多出現: D0807426（13 ペアに登場）
```

「D0807426 が 13 件のペアに登場する」とは、その特許が異なる 13 件の意匠特許と共引用された（共に審査で参照された）ことを意味する。
