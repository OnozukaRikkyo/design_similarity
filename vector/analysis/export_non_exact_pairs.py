...
identical / exact / same を含まないペアの画像を出力する。

判定基準:
    reason に identical / exact / same のいずれも含まれない → 完全一致でない
...
EXACT_RE = re.compile(r"\b(identical|exact|same)\b", re.IGNORECASE)
...
def is_non_exact(reason: str) -> bool:
    return not EXACT_RE.search(reason)
...
def load_non_exact(
    ...
    return [r for r in recs if is_non_exact(r.get("reason", ""))]
...
def plot_non_exact_pair(rec: dict, out_path: Path) -> None:
    ...
    レイアウト（2 行 × 3 列）:
      ...
      Row 0, Col 2-3 : Expected（ターゲット）画像
      Row 0, Col 4   : 統計情報
...
def main() -> None:
    ...
    recs.sort(key=lambda r: (r["rank"], -r["similarity"]))
    print(f"完全一致でない Yes ペア: {len(recs)} 件")
