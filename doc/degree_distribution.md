...
CCDF の補助線は以下の 3 パラメータで制御する。

| パラメータ | デフォルト | 意味 |
|-----------|-----------|------|
| `k_fit_min` | 10 | フィットに使う次数の下限（初期カーブが終わり直線域が始まる点） |
| `k_fit_max` | 200 | フィットに使う次数の上限（末端の落ち込みが始まる前の点） |
| `line_shift` | 3.0 | 補助線をデータ点から離すための右シフト係数（log 空間での乗数） |

**フィット**: `unique_k ∈ [k_fit_min, k_fit_max]` の範囲のみで OLS を実行し、傾き（α = γ−1）を求める。

**描画**: 補助線の x 範囲を `[k_fit_min × s, k_fit_max × s]` にシフトし、
y 値を `C · s^α · k^{−α}` で再計算することで、傾きを保ったまま右へ平行移動する（s = `line_shift`）。
データ点と補助線が重ならず、傾きを視覚的に比較しやすくなる。
...
# CCDF フィット範囲を手動指定（初期カーブ終了〜末端落ち込み前）
python plot_indegree.py --k-fit-min 10 --k-fit-max 200

# 補助線のデータ点からの離し量を調整（デフォルト 3.0）
python plot_indegree.py --k-fit-min 10 --k-fit-max 200 --k-line-shift 4.0
```
