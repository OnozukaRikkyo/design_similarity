...
─────────── グラフ解析（update_downstream.py デフォルト実行、--no-graph でスキップ）────────

          ▼ Step K: graph/graph_analysis.py
  graph/output/D18/triadic_scored.jsonl      ← 全三角形スコア

          ▼ Step L: graph/extract_high_sim_triads.py
  graph/output/D18/high_sim_triads/

          ▼ Step N: graph/verify/discord_analysis.py  ← K に依存
  graph/output/D18/verify/
    fp.csv / fn.csv                          ← ★ Step M への橋渡し

          ▼ Step M: graph/verify/wcc_scoring.py       ← K, N に依存
  graph/output/D18/verify/
