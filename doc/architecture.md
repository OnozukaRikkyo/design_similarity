...
| `design_similarity/vector/run_pipeline.py` | **ベクトル検索パイプライン**（クラス別ランク検索） | 本ディレクトリ |
...
> **注意:** `run_pipeline.py` という名前のファイルが 2 つ存在する。  
> 上流パイプライン → `design_similarity/run_pipeline.py`  
> ベクトル検索 → `design_similarity/vector/run_pipeline.py`
...
─────────── ベクトル検索パイプライン（vector/run_pipeline.py）────────────────
...
/home/sonozuka/
  design_similarity/               ← 本リポジトリ
    ...
    vector/
      run_pipeline.py              ← ベクトル検索パイプライン（Step 1〜5）
      analysis/                    ← 分析スクリプト群
...
### 3. `run_pipeline.py` が 2 つある

同名ファイルが異なる場所に存在し役割が全く異なる。  
`design_similarity/run_pipeline.py` → 上流（エッジ・LLM 判定）  
