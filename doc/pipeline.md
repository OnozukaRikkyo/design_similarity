...
| 入力 | `/mnt/eightthdd/uspto/similarity_results/{year}.jsonl` | JSONL（P4 のみ） |
...
| | パス | 形式 |
|---|---|---|
| 入力 | `/mnt/eightthdd/uspto/similarity_results/*.jsonl` | JSONL |
| 入力 | `/mnt/eightthdd/uspto/data/*.csv` | CSV |
| 出力 | `debug/yes_json/<src>__<tgt>.json` | JSON |
| 出力 | `debug/yes_image_pair/<src>__<tgt>.png` | PNG |
| キャッシュ | `debug/_patent_index.pkl` | pickle |
...
（スクリプトと同ディレクトリ）
  ...
  debug/yes_json/              ← STEP 4 出力（Yes レコード JSON）
  debug/yes_image_pair/        ← STEP 4 出力（Yes 横並び画像）
  debug/_patent_index.pkl      ← STEP 4 キャッシュ
...
python visualize_ergm_network.py --sim-dir /mnt/eightthdd/uspto/similarity_results  # Gemini Yes 重ね描き
```
