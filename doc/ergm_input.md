...
`qwen_similarity_results/*.jsonl`（STEP 3 出力）と `attributes.txt` を結合することで、以下を検証できる。
...
| 4（統合） | 類似判定 Yes ペアと ERGM 結果の突合 | `qwen_similarity_results/`, `attributes.txt` |
...
| `--sim-dir` | `/mnt/eightthdd/uspto/qwen_similarity_results` | 類似判定 JSONL のディレクトリ |
...
`qwen_similarity_results/*.jsonl` の `source`/`target` フィールドは特許ID文字列（例: `D0535736`）。  
